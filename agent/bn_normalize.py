...
def verbalize(text: str) -> str:
    ...
    text = _RE_DATE.sub(
        lambda m: (
            date_to_bn_words(int(m.group(1)), int(m.group(2)), int(m.group(3)))
            + ("ে" if (m.group(4) or "").strip().endswith("ে") else "")
        ),
        text,
    )
    text = _RE_TIME_RANGE.sub(
        lambda m: (
            f"{time_to_bn_words(int(m.group(1)), int(m.group(2)))} থেকে "
            f"{time_to_bn_words(int(m.group(3)), int(m.group(4)))} পর্যন্ত"
        ),
        text,
    )
    ...
    text = _RE_DECIMAL.sub(
        lambda m: f"{number_to_bn_words(int(m.group(1)))} দশমিক {digits_one_by_one(m.group(2))}",
        text,
    )
