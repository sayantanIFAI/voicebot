...
def synthetic(n_calls, verbose=False, voice_changes=False):
    ...
    for k in range(n_calls):
        ...
        if k % 2:
            ...
            if hits:
                ...
                missed += 1
            if verbose:
                print(f"call {k}: caller onset {on / SR:.2f}s, first event "
                      f"{'+%.0f ms' % lat[-1] if hits else 'MISSED'}, ERLE at onset {fd.echo.canceller.erle_db:.1f} dB")
        else:
            fd, cleaned, events = run(mic, far, dur_each=seg)
            own_audio_triggers += len(events)
            if verbose and events:
                print(f"call {k}: OWN-AUDIO TRIGGER at " + ", ".join(f"{e.detected_at_sample / SR:.2f}s" for e in events))
            erles.append(erle_db(mic[-3 * SR:], cleaned[-3 * SR:]))
        if (k + 1) % 20 == 0:
            print(f"  {k + 1}/{n_calls}", file=sys.stderr, flush=True)
    print(f"echo-only calls: {len(erles)}  own-audio triggers: {own_audio_triggers}"
    ...
    if lat:
        print(f"detection latency ms: median {np.median(lat):.0f}, p95 {np.percentile(lat, 95):.0f}, max {np.max(lat):.0f}"
              "   (budget: p95 <= 300 ms)")
...
def _load(path):
    with wave.open(path, "rb") as w:
        assert w.getframerate() == SR and w.getnchannels() == 1 and w.getsampwidth() == 2, f"{path}: need 16 kHz mono 16-bit"
...
def main():
    ...
    ap.add_argument("--voice-changes", action="store_true",
