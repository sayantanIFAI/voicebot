...
FORBIDDEN_KEYS = frozenset({"result", "results", "value", "values", "reading", "readings",
                            "interpretation", "finding", "findings", "diagnosis", "report_text",
                            "outcome_value", "impression"})
...
CONFIRMED_ALLOWLIST = {"doctor_name", "date", "time_slot", "test_name", "test_names", "confirmation_id",
                       "new_date", "new_time_slot", "relationship", "department"}
...
def _load(row: CallRecord) -> dict:
    return {
        "call_id": row.call_id, "channel": row.channel, "patient_ref": row.patient_id,
        "started_at": _iso(row.started_at), "ended_at": _iso(row.ended_at),
        "languages": [x for x in row.languages.split(",") if x],
        "intents": json.loads(row.intents_json), "actions": json.loads(row.actions_json),
        "confirmed": json.loads(row.confirmed_json),
        "history_statements": json.loads(row.history_statements_json),
        "escalation_reason": row.escalation_reason, "outcome": row.outcome,
        "disclosure_version": row.disclosure_version, "last_seq": row.last_seq,
    }
...
def _name_fit(spoken: str, stored: str) -> str | None:
    ...
    if all(t in stored_t for t in spoken_t):
        return "exact"
    if all(len(phonetic_key(t)) >= MIN_PHONETIC_KEY_LEN and any(phonetic_match(t, u) for u in stored_t) for t in spoken_t):
        return "phonetic"
    return None
...
def _local_events(db: Session, patient: Patient, now: datetime.datetime) -> list[dict]:
    ...
    for t in tb:
        ev.append({"id": f"test_booking:{t.id}", "kind": "test_booking", "at": t.date, "source": "local", "as_of": as_of,
                   "fields": {"confirmation_id": t.confirmation_id, "test_name": t.lab_test.name if t.lab_test else None,
    ...
    for cid in {t.confirmation_id for t in tb}:
        for r in db.query(LabReport).filter(LabReport.confirmation_id == cid, LabReport.status == "ready").all():
            ev.append({"id": f"report_ready:{r.id}", "kind": "report_ready", "at": _iso(r.ready_at), "source": "local",
                       "as_of": as_of, "fields": {"confirmation_id": cid, "ready_at": _iso(r.ready_at)}})
    for c in db.query(CallRecord).filter((CallRecord.patient_id == patient.id) | (CallRecord.caller_phone == patient.phone)).all():
        ev.append({"id": f"contact:{c.id}", "kind": "contact", "at": _iso(c.started_at), "source": "local", "as_of": as_of,
                   "fields": {"channel": c.channel, "outcome": c.outcome,
                              "intents": json.loads(c.intents_json)[-3:]}})
    return ev
...
def timeline(db: Session, patient_id: int, caller_phone: str, call_id: str) -> dict:
    ...
    return {"success": True, "patient_ref": patient_id, "as_of": _iso(now), "events": events,
...
def test_status(db: Session, patient_id: int, lab_test_id: int, today: datetime.date | None = None) -> dict:
    ...
    last = (db.query(TestPerformance).filter_by(patient_id=patient_id, lab_test_id=lab_test_id)
            .order_by(TestPerformance.performed_on.desc()).first())
    ...
    return {"known": True, "last_performed_on": last.performed_on, "due": today >= due_on,
            "interval_days": interval.interval_days, "due_on": due_on.isoformat(),
...
def get_preferences(db: Session, patient_id: int, caller_phone: str) -> dict:
    ...
    return {"success": True, "preferences": {k: getattr(row, k) for k in
            ("branch", "collection_address", "delivery_channel", "accessibility_mode", "language_bias")
            if getattr(row, k) is not None}, "updated_at": _iso(row.updated_at)}
...
def set_preferences(db: Session, patient_id: int, caller_phone: str, **fields) -> dict:
    ...
    if patient is None or not caller_phone or not authorize_disclosure(db, patient, caller_phone):
        return {"success": False, "reason": "not_authorized"}
    allowed = {"branch": None, "collection_address": None, "delivery_channel": DELIVERY_CHANNELS,
               "accessibility_mode": ACCESSIBILITY_MODES, "language_bias": LANGUAGES}
    clean = {}
...
def recent_interactions(db: Session, caller_phone: str, limit: int = 3, exclude_call_id: str | None = None) -> list[dict]:
    ...
    for c in q.order_by(CallRecord.started_at.desc()).limit(limit).all():
        ...
        items.append({"at": _iso(c.started_at), "channel": c.channel, "intent": intents[-1] if intents else None,
                      "outcome": c.outcome})
    for m in db.query(SmsOutbox).filter(SmsOutbox.to_phone == caller_phone).order_by(SmsOutbox.created_at.desc()).limit(limit).all():
...
def search_bookings(db: Session, caller_phone: str, *, phone: str | None = None, name: str | None = None,
                    approx_date: str | None = None, date_window_days: int = 3, test_name: str | None = None,
    ...
    for a in rows:
        ...
        if not patient or not caller_phone or not authorize_disclosure(db, patient, caller_phone):
            continue
        doctor = db.get(Doctor, a.doctor_id)
        cands.append({"kind": "appointment", "confirmation_id": a.confirmation_id, "date": a.date,
                      "time_slot": a.time_slot, "doctor_name": doctor.name if doctor else None, "test_name": None,
                      "name_basis": basis})
    ...
    for t in tq.all():
        names = [t.lab_test.name] + [x for x in (t.lab_test.aliases_bn or "").split("|")
                                     + (t.lab_test.aliases_hi or "").split("|") if x]
        ...
        if not patient or not caller_phone or not authorize_disclosure(db, patient, caller_phone):
            continue
        cands.append({"kind": "test_booking", "confirmation_id": t.confirmation_id, "date": t.date,
                      "time_slot": None, "doctor_name": None, "test_name": t.lab_test.name, "name_basis": basis})
    ...
    if len(matches) == 1:
        ...
        return {"status": "single", "matches": matches, "ignored_filters": ignored,
                "needs_confirmation": matches[0].get("name_basis") == "phonetic"}
    distinguishing = [f for f in ("date", "doctor_name", "test_name", "time_slot")
                      if len({m[f] for m in matches}) > 1]
