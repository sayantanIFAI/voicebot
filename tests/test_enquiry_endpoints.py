...
def test_prep_merge_endpoint(clinic_client):
    result = clinic_client.post("/api/v1/tests/prep/merge",
                                json={"test_names": ["Blood Sugar Fasting", "Lipid Profile"]}).json()
...
def test_home_collection_endpoint(clinic_client):
    eligible = clinic_client.get("/api/v1/home-collection/eligibility",
                                 params={"test_name": "Uric Acid", "postal_code": "700091"}).json()
    assert eligible["found"] and eligible["eligible"] is True

    not_eligible = clinic_client.get("/api/v1/home-collection/eligibility",
                                     params={"test_name": "ECG", "postal_code": "700091"}).json()
...
def test_insurance_coverage_endpoint(clinic_client):
    result = clinic_client.get("/api/v1/insurance/coverage",
                               params={"policy_number": "DEMO-POLICY-001"}).json()
    assert result["found"] and result["covered"]


def test_prescription_requirement_endpoint(clinic_client):
    required = clinic_client.get("/api/v1/tests/HIV Test (ELISA)/prescription-requirement",
                                 params={"lang": "en"}).json()
...
def test_out_of_scope_and_callback_endpoints(clinic_client):
    oos = clinic_client.post("/api/v1/calls/out-of-scope",
                             json={"call_id": "c1", "caller_question": "Do you sell medicine?"}).json()
    assert oos["recorded"]

    cb = clinic_client.post("/api/v1/callbacks", json={
        "phone": "9111111111", "call_id": "c1", "requested_window": "tomorrow morning"}).json()
    assert cb["success"]
