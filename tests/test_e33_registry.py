...
def test_test_status_is_behind_the_same_gate_and_states_due_only_from_an_approved_interval(api):
    ...
    denied = api.get(f"/api/v1/patients/{ref}/test-status", params={"test_name": "HbA1c", "caller_phone": "0", "call_id": call}).json()
    assert denied["reason"] == "not_verified"
    verify(api, call, ref, dob="1962-01-09", name="Sunita Devi")
    s = api.get(f"/api/v1/patients/{ref}/test-status", params={"test_name": "HbA1c", "caller_phone": "0", "call_id": call}).json()
    assert s["success"] and s["known"] and s["due"] is True and s["approved_by"] == "SAMPLE DATA"
    kft = api.get(f"/api/v1/patients/{ref}/test-status", params={"test_name": "Kidney Function Test (KFT)", "caller_phone": "0", "call_id": call}).json()
...
def test_the_timeline_carries_medicines_tests_and_appointments_with_names_in_the_callers_script(api):
    ...
    assert med["fields"]["medicine_name"] == "Levothyroxine" and med["fields"]["medicine_name_bn"] and med["fields"]["medicine_name_hi"]
...
def test_a_date_of_birth_with_a_name_finds_the_record(api):
    r = api.post("/api/v1/patients/find", json={"call_id": new_call(), "dob": "1962-01-09", "name": "Sunita Devi"}).json()
    assert r["status"] == "single"


def test_a_wrong_pair_finds_nobody_and_a_lone_name_is_insufficient(api):
    assert api.post("/api/v1/patients/find", json={"call_id": new_call(), "dob": "1962-01-10", "name": "Sunita Devi"}).json() == {"status": "none"}
    assert api.post("/api/v1/patients/find", json={"call_id": new_call(), "name": "Sunita Devi"}).json() == {"status": "insufficient"}
...
def _events(c, call, items, phone="9830023456"):
    for seq, (kind, payload) in enumerate(items, start=1):
        assert c.post(f"/api/v1/calls/{call}/events", json={"seq": seq, "kind": kind, "payload": payload, "caller_phone": phone}).json()["applied"]


def test_closing_a_call_writes_history_rows_a_cache_entry_and_an_audit_row(api):
    ref, call = ref_of(api, DEBASHIS), new_call()
    _events(api, call, [("start", {"channel": "voice"}), ("patient", {"patient_ref": ref}), ("intent", {"intent": "book_appointment"}),
    ...
    assert ctx["cached"] and ctx["cached"][0]["outcome"] == "completed" and ctx["cached"][0]["actions"][0]["name"] == "booking_confirmed"
...
def test_a_retried_end_event_does_not_double_the_history(api):
    ref, call = ref_of(api, RAVI), new_call()
    _events(api, call, [("patient", {"patient_ref": ref}), ("action", {"name": "tests_booked", "ref": "K1"}), ("end", {"outcome": "completed"})])
...
def test_the_cache_is_kept_for_one_day_and_an_expired_entry_is_never_used(api):
    ref, call = ref_of(api, RAVI), new_call()
    _events(api, call, [("patient", {"patient_ref": ref}), ("intent", {"intent": "book_test"}), ("end", {"outcome": "abandoned"})],
            phone="9830045678")
...
def test_every_identification_verification_and_history_read_is_audited(api):
    ...
    assert any(e["action"] == "identify" for e in api.get("/api/v1/audit", params={"action": "identify"}).json()["entries"])
...
def test_changing_a_message_takes_effect_without_a_deploy_and_bumps_the_version(api):
    ...
    r = api.put("/api/v1/agent/messages", json={"key": "thanks_ack", "lang": "en", "text": "Thanks for letting me know.", "updated_by": "clinic-lead"}).json()
    ...
    same = api.put("/api/v1/agent/messages", json={"key": "thanks_ack", "lang": "en", "text": "Thanks for letting me know."}).json()
    assert same["version"] == r["version"]                                      # no change, no new version
    assert any(e["action"] == "message_changed" and e["actor"] == "operator"
...
def test_an_invalid_message_is_refused_and_an_inactive_one_is_not_served(api):
    assert api.put("/api/v1/agent/messages", json={"key": "x", "lang": "fr", "text": "bonjour"}).json()["success"] is False
    assert api.put("/api/v1/agent/messages", json={"key": "x", "lang": "en", "text": "  "}).json()["success"] is False
    assert api.put("/api/v1/agent/messages", json={"key": "x", "lang": "en", "text": "y" * 601}).json()["success"] is False
