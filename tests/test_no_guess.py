...
from agent.confidence_gate import (
    LOW, UNAVAILABLE, VERIFIED, confidence_state, needs_entity_readback, should_withhold_factual_answer,
)
...
@pytest.mark.parametrize("intent", ["test_rate", "doctor_availability", "test_prep", "clinic_faq",
...
_EMERGENCY = {
    "en": ["I have chest pain", "he is not breathing", "my father fainted", "she is unconscious", "there is heavy bleeding",
           "he is having a seizure", "my child swallowed poison", "I think it is a heart attack", "please send an ambulance",
           "this is an emergency", "he cannot breathe", "there was a road accident", "I want to kill myself",
           "she can't breathe properly", "he collapsed", "bleeding won't stop", "snake bite", "possible stroke"],
    "bn": ["আমার বুকে ব্যথা করছে", "বাবা অজ্ঞান হয়ে গেছে", "শ্বাসকষ্ট হচ্ছে", "প্রচুর রক্ত পড়ছে", "খিঁচুনি হচ্ছে",
           "অ্যাম্বুলেন্স পাঠান", "হার্ট অ্যাটাক মনে হচ্ছে", "দুর্ঘটনা হয়েছে", "বিষ খেয়েছে", "এটা ইমার্জেন্সি"],
    "hi": ["मेरे सीने में दर्द है", "वह बेहोश हो गया", "सांस नहीं ले पा रहे", "खून बहुत बह रहा है", "दौरा पड़ा है",
           "एम्बुलेंस भेजिए", "दिल का दौरा पड़ा", "दुर्घटना हो गई", "ज़हर खा लिया", "यह इमरजेंसी है"],
}
_ORDINARY = [
    "what is the price of a CBC test", "book an appointment with Dr Sen tomorrow", "when does the clinic open",
    "I want to cancel my appointment", "is fasting required for the thyroid test", "my report is ready or not",
    "আমার সিবিসি টেস্টের দাম কত", "কাল ডাক্তার সেনের সঙ্গে অ্যাপয়েন্টমেন্ট চাই", "ক্লিনিক কখন খোলে",
    "मेरा टेस्ट कब हुआ था", "कल डॉक्टर सेन का अपॉइंटमेंट चाहिए", "क्लिनिक कब खुलता है", "hello good morning",
]
...
def env(m, monkeypatch, tmp_path):
    ...
    state = {"text": "what is the price of a CBC test", "lang": "en", "agreement": 0.9, "decoder": "rnnt",
             "handoffs": [], "resolved": 0, "answered": [], "intent": "test_rate", "slots": {"test_name": "CBC"}}
...
@pytest.mark.asyncio
@pytest.mark.parametrize("lang,text", [("en", "my father is not breathing"), ("bn", "বাবা অজ্ঞান হয়ে গেছে"),
...
def clinic(tmp_path):
    ...
    saved = {k: sys.modules.pop(k) for k in ("main", "db", "models", "seed", "booking_service", "booking_migrate",
                                              "enquiry_service", "enquiry_migrate", "i18n_content", "phonetic_match",
                                              "patient_context", "disclosure") if k in sys.modules}
    from fastapi.testclient import TestClient
    import main as clinic_main
    with TestClient(clinic_main.app) as c:
        yield c
    for k in list(sys.modules):
        if k in saved or k in ("main", "db", "models", "seed", "booking_service", "booking_migrate", "enquiry_service",
...
def test_a_sound_alike_doctor_is_suggested_never_resolved_on_any_endpoint(clinic):
    ...
    hold = clinic.post("/api/v1/bookings/hold", json={"doctor_name": "Nukharji", "date": "2026-10-01", "time_slot": "10:00"}).json()
...
def test_with_a_token_configured_the_api_refuses_a_caller_without_it(clinic, monkeypatch):
    ...
    assert clinic.get("/api/v1/tests/search", params={"name": "CBC"},
                      headers={"Authorization": "Bearer wrong"}).status_code == 401
    ok = clinic.get("/api/v1/tests/search", params={"name": "CBC"},
                    headers={"Authorization": "Bearer s3cret-token-for-test"})
...
def test_patient_data_endpoints_are_behind_the_token_too(clinic, monkeypatch):
    monkeypatch.setenv("CLINIC_API_TOKEN", "s3cret-token-for-test")
    for path in ("/api/v1/patients/identify?phone=9876543210", "/api/v1/bookings/lookup?phone=9876543210",
...
def test_confirming_the_same_hold_twice_returns_the_same_booking_not_a_second_one(clinic):
    ...
    body = {"hold_token": hold["hold_token"], "doctor_id": hold["doctor_id"], "date": date, "time_slot": slot,
            "patient_name": "Test Patient", "phone": "9876500001", "caller_phone": "9876500001"}
...
def test_a_different_phone_cannot_replay_someone_elses_confirmed_hold(clinic):
    ...
    hold = clinic.post("/api/v1/bookings/hold", json={"doctor_name": "Sen", "date": date, "time_slot": slot}).json()
    body = {"hold_token": hold["hold_token"], "doctor_id": hold["doctor_id"], "date": date, "time_slot": slot,
            "patient_name": "Test Patient", "phone": "9876500002", "caller_phone": "9876500002"}
