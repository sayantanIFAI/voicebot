...
FULL = {"doctor_name": "Sen", "date": "2026-10-01", "time_slot": "10:00", "patient_name": "Ravi Das",
        "phone": "9876543210"}
...
@pytest.mark.parametrize("action", ["reschedule_appointment", "cancel_appointment", "add_test_booking", "lookup_booking"])
...
@pytest.mark.parametrize("fields", [["doctor_name", "date", "time_slot"], ["date", "time_slot"], ["doctor_name", "date"],
                                    ["doctor_name", "time_slot"], ["patient_name", "phone"], ["test_names", "date"]])
...
def test_the_english_wording_reads_as_one_natural_clause():
    assert grouped_prompt(["doctor_name", "date", "time_slot"], "en") == "For the appointment, which doctor, which day and what time?"
    assert grouped_prompt(["date", "time_slot"], "en") == "For the appointment, which day and what time?"
    assert grouped_prompt(["patient_name", "phone"], "en") == "May I have the patient's name and a phone number for the confirmation?"
...
CALLERS = [   # (opening utterance's slots, how they answer)
    ({}, "cooperative"), ({}, "partial"),
    ({"doctor_name": "Sen"}, "cooperative"), ({"doctor_name": "Sen"}, "partial"),
...
def test_under_a_one_question_policy_the_turn_count_is_exactly_the_baseline(state):
    ...
    for opening, persona in CALLERS:
        assert (_questions_to_complete("book_appointment", opening, persona, q, FULL)
                == _questions_to_complete("book_appointment", opening, persona, 1, FULL))
