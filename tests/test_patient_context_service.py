...
def svc(monkeypatch):
    ...
    for mod in ("main", "db", "models", "seed", "booking_service", "booking_migrate", "enquiry_migrate",
                "i18n_content", "patient_context", "phonetic_match", "disclosure"):
...
def _book_appointment(s, patient, days_ahead=10, phone=None):
    ...
    booked = s.bs.confirm_booking(s.db, hold["hold_token"], doc.id, date, slot, patient.name, patient.phone,
                                  phone or patient.phone, patient_age=patient.age)
...
def test_a_call_record_is_built_as_events_arrive(svc):
    pc = svc.pc
    assert pc.record_call_event(svc.db, "c1", 1, "start", {"channel": "voice", "disclosure_version": "1.0-draft"}, "9000000001")["applied"]
    pc.record_call_event(svc.db, "c1", 2, "language", {"language": "bn"})
    pc.record_call_event(svc.db, "c1", 3, "intent", {"intent": "book_appointment"})
    pc.record_call_event(svc.db, "c1", 4, "confirmed", {"slots": {"doctor_name": "Dr Sen", "date": "2030-01-14",
                                                                 "phone": "9000000001", "patient_name": "Asha Das"}})
...
def test_the_history_statement_ids_and_escalation_are_recorded(svc):
    ...
    assert rec["history_statements"] == ["appointment:1", "cannot_see"] and rec["escalation_reason"] == "caller_requested"
...
def test_a_name_settles_a_shared_number_and_an_age_narrows_but_never_picks(svc):
    ...
    assert svc.pc.resolve_named(svc.db, "9000000003", "Asha") == {"status": "single", "patient_ref": asha.id, "basis": "exact"}
    ...
    assert svc.pc.resolve_named(svc.db, "9000000003", "Zoya")["status"] == "none"    # sounds like "Asha" once vowels fold away: NOT a match
...
def test_every_read_is_audited_and_attributed_to_the_call(svc):
    ...
    assert len(rows) == 1 and rows[0].call_id == "call-77" and rows[0].kind == "timeline_read" and rows[0].patient_id == p.id
...
def test_no_event_may_carry_a_clinical_result(svc, monkeypatch):
    ...
    bad = [{"id": "test_performed:1", "kind": "test_performed", "at": "2030-01-01", "source": "lis", "as_of": "x",
...
def _perform(svc, patient, test_name, on):
    ...
    svc.db.add(svc.m.TestPerformance(patient_id=patient.id, lab_test_id=test.id, performed_on=on,
                                     source="lis", created_at=PINNED_NOW))
    svc.db.commit()
    return test
...
def test_an_unknown_test_history_says_unknown(svc):
    ...
    assert svc.pc.test_status(svc.db, p.id, test.id, TODAY) == {"known": False, "last_performed_on": None, "due": None, "interval_days": None}
...
def test_due_is_computed_only_from_a_clinician_approved_interval(svc):
    ...
    test = _perform(svc, p, "CBC", "2029-06-01")
    svc.db.add(svc.m.RetestInterval(lab_test_id=test.id, interval_days=90, approved_by="Dr Sen", approved_at="2029-01-01"))
    svc.db.commit()
    s = svc.pc.test_status(svc.db, p.id, test.id, TODAY)
...
def test_preferences_are_stored_against_the_patient_and_read_back(svc):
    ...
    r = svc.pc.set_preferences(svc.db, p.id, "9000000030", delivery_channel="sms", language_bias="bn", accessibility_mode="slower")
...
def test_invalid_or_unknown_preferences_are_refused(svc):
    ...
    assert svc.pc.set_preferences(svc.db, p.id, "9000000032", delivery_channel="carrier_pigeon")["reason"] == "invalid_value:delivery_channel"
    assert svc.pc.set_preferences(svc.db, p.id, "9000000032", language_bias="fr")["reason"] == "invalid_value:language_bias"
    assert svc.pc.set_preferences(svc.db, p.id, "9000000032", blood_group="O+")["reason"] == "unknown_field:blood_group"
    assert svc.pc.set_preferences(svc.db, p.id, "9000000032", collection_address="x" * 500)["reason"] == "too_long:collection_address"
...
def test_the_last_three_interactions_across_voice_and_messaging(svc):
    pc = svc.pc
    for i, (cid, outcome) in enumerate([("a", "completed"), ("b", "abandoned"), ("c", "completed"), ("d", "handed_off")]):
        ...
        svc.db.commit()
    svc.db.add(svc.m.SmsOutbox(to_phone="9000000040", template_key="booking_confirmed", message="m", status="queued",
...
def test_a_test_booking_is_found_by_test_name(svc):
    ...
    assert r["status"] == "single" and r["matches"][0]["confirmation_id"] == conf and r["matches"][0]["test_name"] == test.name
