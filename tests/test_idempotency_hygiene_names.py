...
def test_a_retried_sms_with_the_same_key_is_one_message(api):
    _app, c = api
    body = {"to": "9830011111", "template_key": "confirm", "message": "Your booking is confirmed.",
...
def test_the_same_key_with_a_different_body_is_refused(api):
    _app, c = api
    c.post("/api/v1/callbacks", json={"phone": "9830022222", "call_id": "c1", "requested_window": "morning"},
           headers={"Idempotency-Key": "k-cb-1"})
    r = c.post("/api/v1/callbacks", json={"phone": "9830022222", "call_id": "c1", "requested_window": "evening"},
               headers={"Idempotency-Key": "k-cb-1"})
...
def test_the_same_key_on_two_endpoints_is_two_keys(api):
    _app, c = api
    a = c.post("/api/v1/callbacks", json={"phone": "9830033333", "call_id": "c2", "requested_window": "morning"},
               headers={"Idempotency-Key": "shared"})
    b = c.post("/api/v1/calls/out-of-scope", json={"call_id": "c2", "caller_question": "q", "reason_code": "out_of_scope"},
               headers={"Idempotency-Key": "shared"})
...
def test_without_a_key_the_same_sms_within_the_window_is_still_one_message(api):
    _app, c = api
    body = {"to": "9830044444", "template_key": "confirm", "message": "Hello there.", "related_confirmation_id": "KCD-T2"}
...
def test_without_a_key_the_same_out_of_scope_question_is_recorded_once(api):
    ...
    a, b = c.post("/api/v1/calls/out-of-scope", json=body).json(), c.post("/api/v1/calls/out-of-scope", json=body).json()
    assert a["id"] == b["id"]
