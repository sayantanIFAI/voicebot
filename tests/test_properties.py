...
from agent.call_score import _COUNTED, _FLAGS, BASE_SCORE, MIN_TURNS_TO_SCORE, CallSignals, band_for, score_call  # noqa: E402
...
def test_the_fingerprint_is_stable_and_ignores_key_order():
    ...
    @given(st.dictionaries(st.text(min_size=1, max_size=8), st.one_of(st.integers(), st.text(max_size=8), st.booleans()),
                           max_size=6))
    @FAST
    def check(d):
...
def test_removing_cue_phrases_is_idempotent_and_never_adds_words():
    ...
    @given(st.lists(st.sampled_from(["price", "rate", "fasting", "the", "test", "cbc", "what", "is", "how", "much"]),
                    max_size=10))
    @FAST
    def check(words):
