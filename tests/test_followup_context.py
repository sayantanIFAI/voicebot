...
@pytest.mark.parametrize("value", ["সমে টেস্ট", "এই সমে টেস্ট টাই", "সেম টেস্ট", "এই টেস্ট", "ওই টেস্টটা", "সেই টেস্ট",
                                   "same test", "this test", "the same", "that one", "the test", "यही टेस्ट", "वही टेस्ट", "सेम टेस्ट"])
def test_a_value_that_only_points_is_a_reference_not_a_name(value):
    assert is_reference_only(value)


@pytest.mark.parametrize("value", ["ক্রিয়াটিনিন", "ইউরিক অ্যাসিড টেস্ট", "সেন", "দে", "CBC", "lipid profile test", "एचबीए वन सी", "", None])
...
def test_the_second_line_of_the_live_conversation_uses_the_test_just_discussed():
    slots, resolved = resolve_followup_slot("test_prep", {"test_name": None}, "ফাস্টিং করতে হবে", "bn", ONE, turns_since_last=1)
...
def test_with_nothing_remembered_it_asks():
    assert resolve_followup_slot("test_prep", {"test_name": None}, "do I need to fast", "en", [], turns_since_last=None)[1] is False


def test_too_long_ago_and_no_pointer_it_asks():
    assert resolve_followup_slot("test_prep", {}, "do I need to fast", "en", ONE, turns_since_last=FOLLOWUP_MAX_GAP + 1)[1] is False
    assert resolve_followup_slot("test_prep", {}, "do I need to fast", "en", ONE, turns_since_last=FOLLOWUP_MAX_GAP)[1] is True


def test_two_remembered_tests_is_ambiguous_so_it_asks_even_when_pointing():
    assert resolve_followup_slot("test_prep", {"test_name": "same test"}, "x", "en", TWO, turns_since_last=1)[1] is False
...
def test_a_test_is_never_filled_into_a_doctor_question_or_the_reverse():
    assert resolve_followup_slot("doctor_availability", {}, "when does he sit", "en", ONE, turns_since_last=1)[1] is False
